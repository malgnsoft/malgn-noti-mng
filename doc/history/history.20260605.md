...
## 9. 간트 WBS에 hi-fi 핸드오프 디자인 적용 (`design_handoff_wbs_gantt`)

- 핸드오프(README + wbs.css + 스크린샷)를 분석해 `/wbs`를 **데이터 대시보드 스타일 간트**로 전면 재구현.
- 상단 **KPI**(전체 진척 미터 + 완료/진행중/지연/예정), **툴바**(담당 칩 다중 필터·상태 세그먼트·검색·모두 접기/펼치기·범례), **3계층 트리**(Step→구분→작업, 셰브론 접기), 담당 **아바타**, 상태색 **진척 막대**(채움+%)·1일 **마일스톤**, **롤업 막대**(구분/Step), **주말 음영·일/월 헤더·오늘 기준선**, 막대 **툴팁**.
- 상태 규칙(done/active/plan/late)·집계는 핸드오프 사양. 데이터는 `wbsData.ts`(Step 1·3·5) 유지·트리 변환. 라이트 테마 토큰을 컴포넌트 스코프로 이식.

## 10. WBS 항목 등록/수정/삭제 (D1 CRUD)

- 정적 `wbsData.ts` 기반 WBS를 **D1(`wbs_item` 테이블) + API**로 전환해 항목 CRUD 지원.
- 스키마(`server/db/schema.ts` `wbsItem`) + 마이그레이션 `0001` + 시드(114건, `wbsData` 이관) 원격 적용.
- API: `GET /api/wbs`(목록) · `POST /api/wbs`(등록) · `PATCH /api/wbs/:id`(수정) · `DELETE /api/wbs/:id`(삭제) — Drizzle/D1, dev는 정적 시드 폴백.
- 페이지: 정적 import → `useFetch('/api/wbs')` 반응형으로 전환(프리렌더 제외, SSR). 툴바 **＋작업 추가**, 행 hover **수정/삭제** 버튼, 등록/수정 **모달**(단계·구분·작업명·담당·시작/종료·진척·메모·링크). `wbsSteps`/`wbsStageMeta`는 정적 유지.
- e2e: 생성→수정→삭제 D1 반영 확인.

## 11. 간트 WBS 데이터·진척 정합 + UI 보정

- **화면 단위 분해**: Step 5(서비스 개발)를 실제 화면(본수)으로 분해 — 사용자단 47 · 관리자단 18(설계·API 제외). 구분을 `사용자 · …`/`관리자 · …` 하위 영역으로.
- **이미지 기준 현행화**: Step 1·Step 3을 제공 이미지대로 담당자(컨설팅팀/김덕조/김경은/안병훈/김도형)·일자·진척·메모 반영. 날짜 미정 항목 지원(막대 없이 `–`).
- **진척 집계 일치**: 전체/단계 진척을 현황판(보드)과 동일 **단계 가중평균(47.5%)**으로(`wbsStageMeta` = 보드 weight·progress). 작업 카운트·구분 롤업은 작업 기준.
- **UI 보정**: 구분 열 좌측 패딩·상하 중앙정렬 · 표 고정 높이 스크롤(하단 가로 스크롤 상시 노출)+월/일 헤더 고정 · 진척율 0% 셀 불투명화 · 제목 “전체 일정” · 담당 커스텀 호버 툴팁 · 메모 아이콘+클릭 팝오버.

## 12. 대시보드 프로젝트 개요 추가

- 히어로 아래 “프로젝트 개요” 2단 카드 — 목표(+지원 채널 칩) · 기획 방향(불릿). CLAUDE.md §1 기준.

## 13. malgn-noti `/wbs` 제거 (WBS는 mng로 이관)

- **malgn-noti**: `app/pages/wbs.vue` 삭제 + 미들웨어 정리 → 재배포(페이지 제거).
- **malgn-noti-api**: `routes/wbs.ts`·`data/wbs-seed.ts`·`index.ts` 등록·`openapi.ts` 제거 → 재배포(`GET /wbs` 404).
- **R2**: `malgn-noti-files/wbs/wbs.json` 삭제. 각 레포는 wbs 관련만 선별 커밋. mng는 자체 D1이라 영향 없음.

## 14. 재사용 블루프린트 + 스타터 패키지

- [`doc/PROJECT_MANAGEMENT_BLUEPRINT.md`](../PROJECT_MANAGEMENT_BLUEPRINT.md) — 다른 프로젝트 이식용 일반 설계도(아키텍처·메뉴·화면·D1 스키마·API·디자인 토큰·셋업/배포·치환 체크리스트).
- 필수 스켈레톤 압축본 `project-mng-starter.zip`(설정 + `app/` + `server/` + 블루프린트, 빌드 산출물·프로젝트 데이터 제외).
